...
export default defineComponent({
  ...
  methods : {
    ...
    loadDicValue(typeCode) {
      myAxios.get("/api/dicvalue/" + typeCode, {}).then( resp => {
    ...
    checkPhone(rule, value, callback) {
      ...
      if (phone) {
        myAxios.get("/api/clue/" + phone, {}).then(resp => {
      ...
      this.$refs.clueRefForm.validate( (isValid) =>  {
        if (isValid) {
          ...
          if (this.clueQuery.id > 0) {/*编辑*/
            myAxios.put("/api/clue", formData).then( (resp) => { //获取ajax异步请求后的结果
              console.log(resp);
              if (resp.data.code === 200) {
                //编辑成功，提示一下
            ...
            myAxios.postForm("/api/clue", formData).then( (resp) => { //获取ajax异步请求后的结果
              console.log(resp);
              if (resp.data.code === 200) {
                //录入成功，提示一下
    ...
    loadClue() {
      ...
      if (id) { //id存在，id有值，id不为空，说明的编辑
        myAxios.get("/api/clue/detail/" + id, {}).then(resp => {
          if (resp.data.code === 200) {
